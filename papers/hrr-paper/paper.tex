...
\begin{document}
	...
	% in the abstract
	\begin{abstract}
		Vector Symbolic Architectures (VSA) define a set of operations for association, (compressed) storage, manipulation and retrieval of symbolic concepts, represented as fixed-length vectors from ${\rm I\!R}^N$.
A specific instance of VSA, Holographic Reduced Representations (HRR), has proven to exhibit properties similar to human short-term memory and is, as such, interesting for computational modelling.
		
		In this paper we extend the HRR approach by introducing implicit, topology-preserving, encoding/decoding procedure using radial basis functions.
...
Effectively, encoding implicitly represents the symbol, so no further lookup is needed.
		...
		We show that this newly introduced encoding scheme didn't have a negative impact on memory capacity in comparison with the original author's capacity benchmark.
