...
In figure \ref{perm-b} "ball" is being probed, and the expected result is a Gaussian distribution for 8, mixed with some noise.}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\EndFor		
			\Function{ProbeMemory}{memory, operand}
			\State $result\gets IFFT(FFT(memory)\oslash FFT(operand))$
			\ForAll{$symbol$}
	...
	Just as Plate has done in the original HRR paper \cite{Plate:1995:HolographicReducedRepresentations} we have tested the memory capacity in terms of accuracy of successful recall for variable numbers of bound pairs. Results are shown in figure \ref{fig:capacity}.
		...
		\end{subfigure}
				\begin{subfigure}{\columnwidth}
			\center
			\includegraphics[width=0.8\columnwidth]{img/capacity_scalar_30.png}
			\caption{Memory capacity analysis for binding pairs of one scalar and a random symbol, then probing for the scalar.}
			\label{fig:capacity_scalar}
		\end{subfigure}	
		\begin{subfigure}{\columnwidth}
			\center
			\includegraphics[width=0.8\columnwidth]{img/capacity_coordinate.png}
			\caption{Memory capacity analysis for binding pairs of one 2D coordinate and a random symbol, then probing for the scalar.}
