...
position which, due to the similarity of scalar values, yields results even for
...
to probing the controller with the symbolic constant $no\_object$ which results
...
At tick $-30$, the object enters the agent's visual field and is consequently
centered until tick $-19$.
...
In the right column of figure \ref{fig:experiment}, the target is not currently
in the visual field of the agent.
This is due to reaching its old target at approximately tick $-13$ which leads
to the removal of all current objects and random placement of new objects,
including one target.
As the new object is placed outside of the agent's field of view, the default
action of probing $no\_object$ is performed constantly.
As described, this then results in the encoded default actions $backwards$ for
probing with $left\_wheel$ and $forwards$ for probing with $right\_wheel$---a
clockwise rotation (see figure \ref{fig:experiment:similar2}).

This experiment shows the effectiveness of our approach in a practical
application by encoding symbolic sensing, actuators and actions in a symbolic
controller.
...
\begin{subfigure}{.47\columnwidth}
	...
	\caption{Scene of Agent 1 with three randomly placed items, the blue
...
\begin{subfigure}{.47\columnwidth}
	...
	The agent does not have the target object in his field of view (green
	fan).}
...
\begin{subfigure}{.49\columnwidth}
	...
	\caption{Symbolic sensing with target in visual field results in scalar
	value representing target direction (${\sim}0.55$, almost straight
	forward).}
...
\begin{subfigure}{.49\columnwidth}
	...
	\caption{Symbolic sensing without target in visual field showing the
...
\begin{subfigure}{.49\columnwidth}
	...
	\caption{Target position over time in the visual field of Agent 1.
	At tick $-30$, it appears on the far right edge and then gradually
...
\begin{subfigure}{.49\columnwidth}
	...
	In this case, the old target was reached and the new target is not in
	the field of view yet.}
...
\begin{subfigure}{.49\columnwidth}
	...
	Here, the blue object in \subref{fig:experiment:agent1} is being
	approached.}
...
\begin{subfigure}{.49\columnwidth}
	...
	At tick $-12$ a new target was added (black bar).}
...
\begin{subfigure}{.49\columnwidth}
	...
	\caption{Similarities as described in \subref{fig:experiment:similar1}
...
The left column (\protect\subref{fig:experiment:agent1},
...
\protect\subref{fig:experiment:similar2}) shows an agent without a target in its
